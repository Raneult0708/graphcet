...
const GrafcetRenderer: React.FC<Props> = ({ data }) => {
  ...
  const renderTransition = (trans: GrafcetTransition) => {
    const width = 30; // Length of the horizontal bar
    const isDouble = trans.type === 'divergence_and' || trans.type === 'convergence_and';
    const isOrDivergence = trans.type === 'divergence_or' || trans.type === 'convergence_or';
    
    // Logic for Divergence/Convergence Bar width
    const renderBarWidth = (isDouble || isOrDivergence) && Array.isArray(trans.to) && trans.to.length > 1 ? 200 : width;
    
    // Special handling for shared transitions is complex, simplifying to standard bars centered on x/y
    ...
    return (
      <g key={`trans-${trans.id}`}>
        ...
        {isDouble && (
          <line
            x1={trans.x - 100} // Wide bar for AND
            y1={trans.y - 4} // Slightly above/below logic needs refinement for perfect drawing, using separate entities in data usually
            x2={trans.x + 100}
            y2={trans.y - 4}
            stroke="black"
            strokeWidth="2"
